...
class SettingsPage(ttk.Frame):
    def __init__(self, parent: tk.Misc, repository: SettingsRepository) -> None:
        ...
        self.ai_prompt = LabeledText(body, "AI Prompt", height=7)
        self.cover_letter_prompt = LabeledText(body, "Cover Letter Prompt", height=7)
        self.resume_international = LabeledText(
            body, "User LaTeX Resume Code (International)", height=18, wrap="none"
        )
        self.resume_local = LabeledText(
            body, "User LaTeX Resume Code (Local)", height=18, wrap="none"
        )
        ...
        self.resume_international.grid(row=1, column=0, sticky="nsew", padx=(0, 8))
        self.resume_local.grid(row=1, column=1, sticky="nsew", padx=(8, 0))

        footer = ttk.Frame(self)
        footer.grid(row=3, column=0, sticky="ew", pady=(16, 0))
        footer.columnconfigure(0, weight=1)
        ttk.Button(footer, text="Reload", command=self.load_settings).grid(row=0, column=1, padx=(0, 8))
        ttk.Button(footer, text="Save Settings", command=self.save_settings, style="Primary.TButton").grid(
            row=0, column=2
        )
